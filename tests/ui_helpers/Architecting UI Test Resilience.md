...
This methodology is deeply aligned with the principles of the **Accessibility Tree**, the simplified functional representation of the UI generated by the browser, which defines elements based on their stable semantic properties (Role, Name, and State) rather than volatile structural details.
...
| 4 | State/ARIA Attributes | Condition Verification / Accessibility State | High (Functional Condition) |
| 5 | page.getByText() | Visible Content | Moderate (Subject to copy changes) |
| 6 | Chained/Relative Locators | Contextual Positioning | Moderate (Dependent on Anchor Stability) |
| 7 (Lowest) | CSS, XPath (Partial/Contains) | Structural Pathing | Low (Fallback for Fuzzy Matching) |
