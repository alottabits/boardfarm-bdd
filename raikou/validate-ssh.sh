...
for container in "${CONTAINERS[@]}"; do
    ...
    if docker compose ps "$container" 2>/dev/null | grep -q "Up"; then
        # Check if SSH daemon is running inside container (multiple methods for robustness)
        # Method 1: Check for sshd process using full command line match
        # Method 2: Check if port 22 is listening
        # Method 3: Fallback to simple pgrep
        if docker exec "$container" sh -c 'pgrep -f "[s]shd" > /dev/null 2>&1 || pgrep sshd > /dev/null 2>&1 || (netstat -tlnp 2>/dev/null | grep -q ":22 ") || (ss -tlnp 2>/dev/null | grep -q ":22 ")' 2>/dev/null; then
