...
The `ui_discovery.py` tool is now implemented in `boardfarm/boardfarm3/lib/gui/ui_discovery.py` with **automatic URL pattern recognition** to reduce redundancy in UI maps.
...
# Discover GenieACS UI with pattern detection enabled (default)
python ../boardfarm/boardfarm3/lib/gui/ui_discovery.py \
  --url http://127.0.0.1:3000 \
  --output ./tests/ui_helpers/genie_acs_ui_map.json \
  --username admin \
  --password admin \
  --max-depth 2 \
  --headless
...
```bash
# Enable interaction discovery (discover modals and dialogs)
python ../boardfarm/boardfarm3/lib/gui/ui_discovery.py \
  --url http://127.0.0.1:3000 \
  --output ./tests/ui_helpers/genie_acs_ui_map_with_interactions.json \
  --username admin \
  --password admin \
  --max-depth 2 \
  --headless \
...
# Customize safe button patterns and timeout for interaction discovery
python ../boardfarm/boardfarm3/lib/gui/ui_discovery.py \
  --url http://127.0.0.1:3000 \
  --output ./tests/ui_helpers/genie_acs_ui_map.json \
  --username admin \
  ...
  --headless \
  --discover-interactions \
  --safe-buttons "New,Add,Create,Edit,View,Details,Show" \
  --interaction-timeout 3

# Disable pattern detection (keep all pages separate)
python ../boardfarm/boardfarm3/lib/gui/ui_discovery.py \
  --url http://127.0.0.1:3000 \
  --output ./tests/ui_helpers/genie_acs_ui_map_full.json \
  --username admin \
  --password admin \
  --max-depth 2 \
  --headless \
...
# Adjust pattern detection threshold (require at least 5 similar URLs)
python ../boardfarm/boardfarm3/lib/gui/ui_discovery.py \
  --url http://127.0.0.1:3000 \
  --output ./tests/ui_helpers/genie_acs_ui_map.json \
  --username admin \
...
# Full-featured discovery with all options enabled
python ../boardfarm/boardfarm3/lib/gui/ui_discovery.py \
  --url http://127.0.0.1:3000 \
  --output ./tests/ui_helpers/genie_acs_ui_map_complete.json \
  --username admin \
  --password admin \
  --max-depth 2 \
  --headless \
  --discover-interactions \
  --safe-buttons "New,Add,Edit,View,Show,Cancel,Close" \
...
|--------|------|---------|-------------|
| `--url` | string | (required) | Base URL of the application to crawl |
| `--output` | string | `ui_map.json` | Output file for UI map |
| `--username` | string | None | Login username (optional) |
| `--password` | string | None | Login password (optional) |
| `--login-url` | string | None | Custom login URL (optional) |
| `--max-depth` | int | `3` | Maximum crawl depth |
| `--headless` | flag | `True` | Run browser in headless mode |
| `--no-headless` | flag | - | Run browser with GUI (disables headless) |
| `--no-login` | flag | - | Skip login step |
| `--disable-pattern-detection` | flag | - | Disable URL pattern detection |
| `--pattern-min-count` | int | `3` | Minimum URLs required to form a pattern |
| `--discover-interactions` | flag | - | Enable button click and modal discovery |
| `--safe-buttons` | string | `New,Add,Edit,View,Show,Cancel,Close` | Comma-separated safe button patterns to click |
| `--interaction-timeout` | int | `2` | Seconds to wait for modals after clicking |

#### Output Format with Pattern Recognition
      ...
      "interactions": [
        {
          ...
          "result": {
            ...
            "title": "New Preset",
            "buttons": [
              {"text": "Save", "type": "submit", "css_selector": "button[type='submit']"},
              {"text": "Cancel", "css_selector": "button.cancel"}
            ],
            "inputs": [
              {"type": "text", "name": "name", "placeholder": "Preset name"},
              {"type": "text", "name": "channel"}
            ],
            "selects": [
              {"name": "provision", "options": ["bootstrap", "default"]}
            ]
          }
        }
      ]
  ...
  "url_patterns": [
    {
      ...
      "example_urls": [
        "http://127.0.0.1:3000/#!/devices/06E1AF-SN06E1AFE199B4",
        "http://127.0.0.1:3000/#!/devices/0A08B3-SN0A08B3DA2A1D",
        "http://127.0.0.1:3000/#!/devices/0A4250-SN0A4250DF0848",
        "http://127.0.0.1:3000/#!/devices/2A091C-SN2A091C4974F4",
        "http://127.0.0.1:3000/#!/devices/2A4A90-SN2A4A90A8FBC6"
      ],
      "count": 15,
      "page_structure": {
        "title_pattern": "06E1AF-SN06E1AFE199B4 - Devices - GenieACS",
        "page_type": "home",
        "common_buttons": [
          "Log out",
          "Reboot",
...
The `--discover-interactions` feature enables the tool to click safe buttons on each page to discover modals, dialogs, and dynamic content that wouldn't be visible on initial page load.

**How It Works:**

1. For each page, identify buttons matching safe patterns
2. Click each safe button and wait for modals to appear
3. Capture modal structure (title, buttons, inputs, selects)
4. Close the modal and restore page state
5. Continue with next button

**Safety Controls:**

- **Default Safe Patterns**: `New`, `Add`, `Edit`, `View`, `Show`, `Cancel`, `Close`
- **Excluded Patterns**: `Delete`, `Remove`, `Reset`, `Reboot`, `Submit`, `Save` (potentially destructive)
- **Timeout Protection**: Maximum wait time configurable (default: 2 seconds)
- **State Recovery**: Navigates back to original page URL after each interaction
- **Error Handling**: Graceful recovery with page reload on exceptions

**Use Cases:**

- Discover modal forms for creating/editing resources
- Find confirmation dialogs
- Capture wizard steps and multi-step forms
- Identify dynamic form fields

**Example:**

```bash
# Discover all interactive elements with custom safe patterns
python ../boardfarm/boardfarm3/lib/gui/ui_discovery.py \
  --url http://127.0.0.1:3000 \
  --username admin \
  --password admin \
  --discover-interactions \
  --safe-buttons "New,Add,Create,Edit,View,Details" \
  --interaction-timeout 3 \
  --output ui_map_complete.json
```

**Example Output for a Modal:**

When the tool clicks a "New" button on the Presets page, it captures:
  ...
  "result": {
    ...
    "inputs": [
      ...
      },
      {
        ...
        "placeholder": "",
        "required": false,
        "css_selector": "input[name='channel']"
      }
    ],
    "selects": [
      {
...
This comprehensive data enables the selector and navigation generators to create complete YAML artifacts that include modal interactions.
...
- 19 pages discovered
- 15 device detail pages stored separately
- Large, redundant UI map (~450KB)

**After Pattern Recognition:**

- 19 pages discovered (same coverage)
- 1 pattern detected representing 15 device pages
- Reduced redundancy while preserving examples
- Clear identification of parameterized routes

**With Interaction Discovery:**

- Captures modal dialogs and their form fields
- Discovers hidden UI elements triggered by buttons
- Complete element inventory for selector generation

#### Analyzing Discovery Results

```bash
# View detected patterns
cat ./tests/ui_helpers/genie_acs_ui_map.json | jq '.url_patterns'

# Count total pages
cat ./tests/ui_helpers/genie_acs_ui_map.json | jq '.pages | length'

# View pattern details
cat ./tests/ui_helpers/genie_acs_ui_map.json | jq '.url_patterns[] | {pattern, count, description}'

# View discovered interactions
cat ./tests/ui_helpers/genie_acs_ui_map.json | jq '.pages[] | select(.interactions) | {
  url,
  interaction_count: (.interactions | length),
  interactions: [.interactions[] | {trigger: .trigger.text, modal: .result.title}]
}'

# Count pages with interactions
cat ./tests/ui_helpers/genie_acs_ui_map.json | jq '[.pages[] | select(.interactions)] | length'

# View all modal forms discovered
cat ./tests/ui_helpers/genie_acs_ui_map.json | jq '[.pages[].interactions[]? | select(.result.type == "modal") | {
...
The pattern recognition feature helps identify when new page types are added vs. when new instances of existing patterns appear:

```python
# When comparing UI maps, patterns help distinguish:
# - New pattern detected → New page type added (significant change)
# - Pattern count increased → New instance of existing page (minor change)
# - Pattern structure changed → UI redesign (breaking change)
```

### Testing Pattern Detection

The UI discovery tool includes comprehensive unit tests for the pattern detection functionality.

#### Running Pattern Detection Tests

The tests are located in `boardfarm/unittests/lib/gui/test_pattern_detection.py`:
...
# Run specific test
python -m pytest unittests/lib/gui/test_pattern_detection.py::TestURLPatternDetector::test_genieacs_device_pattern -v
...
- **Simple Pattern Detection**: Detects basic ID patterns like `#!/devices/{device_id}`
- **Threshold Filtering**: Ensures patterns below minimum count aren't detected
- **Multiple Patterns**: Handles multiple different pattern types simultaneously
- **GenieACS Patterns**: Real-world device serial number patterns
- **Page Structure**: Extracts common elements across pattern instances
- **Routing Types**: Both hash-based (`#!/`) and path-based (`/`) routing
- **Custom Thresholds**: Configurable minimum pattern counts

#### Validating Discovery Output

After running UI discovery, validate the output:

```bash
# Check if patterns were detected
cat ./tests/ui_helpers/genie_acs_ui_map.json | jq '.url_patterns | length'
...
cat ./tests/ui_helpers/genie_acs_ui_map.json | jq '.url_patterns[] | {
  ...
  count,
  description,
  example_count: (.example_urls | length)
}'

# Check for expected patterns
cat ./tests/ui_helpers/genie_acs_ui_map.json | jq '.url_patterns[] | select(.pattern == "#!/devices/{device_id}")'

# Validate common page structure
cat ./tests/ui_helpers/genie_acs_ui_map.json | jq '.url_patterns[0].page_structure.common_buttons'
```

#### Expected Results for GenieACS

When running discovery against GenieACS, you should see:

**Pattern Detection:**

- **Pattern**: `#!/devices/{device_id}`
- **Count**: 10-20 device instances (depending on your test data)
- **Common Buttons**: `["Log out", "Reboot", "Reset", "Push file", "Delete"]`
- **Example URLs**: 5 representative device URLs

**Interaction Discovery** (with `--discover-interactions`):

- **Presets Page**: "New" button → "New Preset" modal with name/channel inputs
- **Provisions Page**: "New" button → "New Provision" modal with script textarea
- **Files Page**: "New" button → "New File" modal with file upload input
- **Virtual Parameters Page**: "New" button → "New Virtual Parameter" modal with script editor
- Each modal typically includes "Save" and "Cancel" buttons
...
1. ✅ Create the `ui_discovery.py` tool **with URL pattern recognition**.
