...
**Config flow:** Device receives **merged config** (inventory + env) from `parse_boardfarm_config()`. Device-specific keys (e.g. `dhcp_options`, `gui_fsm_graph_file`) come from this merged dict.
...
**Contains:**
- **Schemas:** Dataclasses, type definitions (e.g. `HTTPResult`, `DHCPOptions`)
- **Helpers:** Console-agnostic functions (e.g. `http_get`, `nmap`)
- **Adapters:** Composable components (e.g. `FsmGuiComponent`)

**Does NOT contain:** Business logic that belongs in use cases, or device-specific I/O that belongs in device classes.

**Example:** `lib/networking.py` has `HTTPResult`, `http_get`, `nmap`, `traceroute_host`.
...
- Topology-specific keys (e.g. `wan_iface`, `lan_iface`)
...
- `environment_def.<device>_presets`: Optional named presets for BDD vocabulary (e.g. per-device scenario presets)
