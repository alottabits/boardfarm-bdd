...
2. The ACS triggers an immediate connection request to the CPE.
3. The CPE sends an `Inform` message to the ACS.
...
8. After completing the boot sequence, the CPE reconnects with the ACS.
9. The CPE informs the ACS that the boot sequence has been completed
10. The ACS acknowledges the Inform and may issue follow-up RPCs to verify device state.
11. The CPE resumes normal operation and periodic Inform sessions.
...
- **4.a CPE Not Connected When Reboot Requested**:
  ...
  1. The ACS attempts to send the Reboot RPC, but the CPE is switched off.
  2. The ACS queues the Reboot RPC as a pending task.
  3. When the CPE is swtiched on, it connects to the ACS.
